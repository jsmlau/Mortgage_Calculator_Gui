...
class MortgageData:
    ...
    def __str__(self):
        ...
        ret_str = ((separator + "\n    loan amount: {}" + "\nannual rate: {:8.2f}%" +
                    "\n    duration: {} years.").format(loan_nice, self.rate, self.years))
        return ret_str

    # static and class helpers -------------------------------
    @classmethod
    def valid_string(cls, the_str):
        if (type(the_str) != str or not (cls.MIN_STRING_LENGTH <= len(
                the_str) <= cls.MAX_STRING_LENGTH)):
            return False
        # else
        return True
    ...
    def valid_principal(cls, prin_in):
        if ((type(prin_in) != float and type(prin_in) != int) or not (
                cls.MIN_LOAN <= prin_in <= cls.MAX_LOAN)):
            return False
        # else)
        return True
    ...
    def valid_rate(cls, rate_in):
        if ((type(rate_in) != float and type(rate_in) != int) or not (
                cls.MIN_RATE <= rate_in <= cls.MAX_RATE)):
            return False
        # else
        return True
    ...
    def valid_years(cls, yrs_in):
        if ((type(yrs_in) != float and type(yrs_in) != int) or not (
    ...
    def valid_payment(cls, pmt_in):
        if ((type(pmt_in) != float and type(pmt_in) != int) or not (
                cls.MIN_PMT <= pmt_in <= cls.MAX_PMT)):
            return False
        # else
        return True
...
class MortgageCalc:
    ...
    def compute_monthly_payment(self):
        ...
        mtg_principal = self.the_loan.get_principal()
        mtg_annual_rate = self.the_loan.get_rate()
        mtg_years = self.the_loan.get_years()
    ...
    def compute_principal(self):
        """ mtg_data assumed to reference a MortgageData object """
        mtg_payment = self.the_loan.get_payment()
        mtg_annual_rate = self.the_loan.get_rate()
        mtg_years = self.the_loan.get_years()
...
# =========================== End MortgageCalc Class =======================

# ============================= MortgageGui Class ==========================
class MortgageGui:
    """ Adaptation of MortgageGui to Mortgage Calculator  """

    # nested enum for loan input choices -----------------------
    class MtgVars(Enum):
        Principle = 1
        Rate = 2
        Year = 3
    ...
    unk_text = ["(error)", "principal", "interest rate", "term (years)",
                "Monthly Payment"]

    # default placeholder to put in entry

    # placeholder_list = ["{} - {}          US$".format(MortgageData.MIN_LOAN, MortgageData.MAX_LOAN),
    #                     "{} - {}                     %".format(
    #                         MortgageData.MIN_RATE, MortgageData.MAX_RATE),
    #                     "{} - {}                   years".format(
    #                         MortgageData.MIN_YRS, MortgageData.MAX_YRS),
    #                     "{} - {}      US$".format(MortgageData.MIN_PMT, MortgageData.MAX_PMT)]

    # some class constants for mortgage gui
    INIT_UNKNOWN = MtgVars.Payment
    FNT_TTL, FNT_BDY, FNT_ENTY_DFLT, TTL_CLR, WRK_CLR, ANS_CLR, OPT_CLR_RAD, BTN_CLR = \
        "gray28", "black", "gray70", "gainsboro", "ghostwhite", "brown2", "gainsboro", "ghostwhite"
    ...
    def __init__(self, mstr_rt=None):
        ...
        self.current_unknown = self.INIT_UNKNOWN  # (start w/PRIN)
        self.answer = 0  # val replaced in update_answer()
        self.answer_txt = ""
        self.options_win = None  # prevent > 1 unk win opened
        ...
        unk_val_header_str = "Mortgage Calculator " + unk_val_txt

        # define state variables - values that affect the GUI behavior
        self.state_cur_unk_int = tk.IntVar()
        self.state_cur_ans_txt = tk.StringVar()
        self.state_cur_instr_txt = tk.StringVar()
        # self.state_cur_ans_txt = tk.StringVar()
        self.state_cur_ans = tk.StringVar()  # answer
        self.state_cur_ans_str = tk.StringVar()  # display answer

        # load state variables with values
        self.state_cur_unk_int.set(unk_val_int)
        self.state_cur_ans_txt.set(unk_val_txt.title())
        self.state_cur_instr_txt.set(unk_val_header_str)
        # self.state_cur_ans_txt.set(self.unk_text[unk_val_int])
        self.state_cur_ans.set(str(self.answer))
        self.state_cur_ans_str.set(str(self.answer_txt))
        ...
        self.container = tk.Frame(self.root, bg=self.WRK_CLR, padx=0)
        self.title_frame = tk.Frame(
            self.container, bg=self.TTL_CLR, padx=15, pady=15)
        self.work_frame = tk.Frame(
            self.container, bg=self.WRK_CLR, padx=15, pady=15)
        self.answer_frame = tk.Frame(
            self.container, bg=self.ANS_CLR, padx=15, pady=15)

        # -------------- header message widget ------------------------
        self.msg_head = tk.Message(self.title_frame)
        self.msg_head.config(font=("Helvetica Neue", 14, "bold"),
                             fg=self.FNT_TTL, bg=self.TTL_CLR, width=900)
        self.msg_head.config(textvariable=self.state_cur_instr_txt)

        # ----------------- some label widgets -------------------
        self.lab_num_prin = tk.Label(self.work_frame, text="Principal ($)", font=(
            "Helvetica Neue", 11), padx=15, pady=15, bg=self.WRK_CLR)

        self.lab_num_rate = tk.Label(self.work_frame, text="Interest Rate (%)", font=(
            "Helvetica Neue", 11), padx=15, pady=15, bg=self.WRK_CLR)
        ...
        self.lab_txt_answer = tk.Label(self.answer_frame, textvariable=self.state_cur_ans_txt, fg="white", font=(
            "Helvetica Neue", 14, "bold"), padx=10, pady=15, bg=self.ANS_CLR)

        self.lab_num_answer = tk.Label(self.answer_frame, textvariable=self.state_cur_ans_str, fg="white", font=(
            "Helvetica Neue", 16, "bold"), padx=10, pady=15, bg=self.ANS_CLR)

        # ----------------- some entry widgets -------------------
        # Principle
        self.enter_prin = tk.Entry(self.work_frame)
        self.enter_prin.config(fg=self.FNT_ENTY_DFLT)
        self.enter_prin.insert(0, MortgageData.get_default_principal())
        self.enter_prin.bind("<FocusIn>", self.focus_in)

        # Rate
        self.enter_rate = tk.Entry(self.work_frame)
        self.enter_rate.config(fg=self.FNT_ENTY_DFLT)
        self.enter_rate.insert(0, MortgageData.get_default_rate())
        self.enter_rate.bind("<FocusIn>", self.focus_in)

        self.enter_years = tk.Entry(self.work_frame)
        self.enter_years.config(fg=self.FNT_ENTY_DFLT)
        self.enter_years.insert(0, MortgageData.get_default_years())
        self.enter_years.bind("<FocusIn>", self.focus_in)

        self.enter_payment = tk.Entry(self.work_frame)
        self.enter_payment.config(fg=self.FNT_ENTY_DFLT)
        self.enter_payment.insert(0, MortgageData.get_default_payment())
        self.enter_payment.bind("<FocusIn>", self.focus_in)

        # ----------------- some button widgets -------------------

        self.but_comp_unknown = tk.Button(
            self.work_frame, text="Calculate", command=self.update_answer, bg="ghostwhite")
        self.but_options = tk.Button(
            self.work_frame, text="More Options", command=self.open_options_win, bg="ghostwhite")
        ...
        self.but_comp_unknown.grid(row=2, column=3)
        self.but_options.grid(row=2, column=0)

        self.lab_num_answer.pack(side=tk.BOTTOM)
        self.lab_txt_answer.pack(side=tk.TOP)
    ...
    def focus_in(self, event):
        ...
        event.widget.config(fg=self.FNT_BDY)
    ...
    def update_answer(self):
        ...
        try:
            if self.current_unknown != self.MtgVars.Principle:
                self.enter_prin.config(state="normal")
            ...
            if self.current_unknown != self.MtgVars.Rate:
                self.enter_rate.config(state="normal")
                val = float(self.enter_rate.get())
                if not self.mtg_calc.set_rate(val):
                    tkmb.showerror("Input Error", "RATE" + RNG_ERR)
                    return

            if self.current_unknown != self.MtgVars.Year:
                self.enter_years.config(state="normal")
                val = float(self.enter_years.get())
                if not self.mtg_calc.set_years(val):
                    tkmb.showerror("Input Error", "RATE" + RNG_ERR)
                    return

            if self.current_unknown != self.MtgVars.Payment:
                self.enter_payment.config(state="normal")
        ...
        try:
            if self.current_unknown == self.MtgVars.Principle:
                self.enter_prin.config(state="normal")
                # Call calculator function and display answer in answer field
                self.compute_show_principal()
                # Display the answer in entry widget

                self.enter_prin.insert(0, self.state_cur_ans.get())
                self.enter_prin.config(state="readonly", fg=self.FNT_BDY)
                ...
                self.enter_rate.insert(0, self.state_cur_ans.get())
                self.enter_rate.config(state="readonly", fg=self.FNT_BDY)

            elif self.current_unknown == self.MtgVars.Year:
                self.enter_years.config(state="normal")
                # Call calculator function and display answer in answer field
                self.compute_show_years()
                # Display the answer in entry widget

                self.enter_years.insert(0, self.state_cur_ans.get())
                self.enter_years.config(state="readonly", fg=self.FNT_BDY)
                ...
                # Display the answer in entry widget
                self.enter_payment.insert(0, self.state_cur_ans.get())
                self.enter_payment.config(state="readonly", fg=self.FNT_BDY)
    ...
    def compute_show_principal(self):
        ...
        # set answer to principal
        self.mtg_calc.compute_principal()
        self.answer = round(self.mtg_calc.get_principal(), 2)

        # set answer
        self.state_cur_ans.set(str(self.answer))

        # set answer_text
        self.answer_txt = "$ " + self.state_cur_ans.get()
        self.state_cur_ans_str.set(str(self.answer_txt))

    def compute_show_rate(self):
        self.enter_rate.delete(0, tk.END)

        # set answer to interest rate
        self.mtg_calc.compute_rate()
        self.answer = round(self.mtg_calc.get_rate(), 2)
    ...
    def compute_show_years(self):
        ...
        # set answer to interest years
        self.mtg_calc.compute_years()
        self.answer = round(self.mtg_calc.get_years(), 2)

        # set answer
        self.state_cur_ans.set(str(self.answer))

        # set answer_text
        self.answer_txt = self.state_cur_ans.get() + " years"
        self.state_cur_ans_str.set(str(self.answer_txt))

    def compute_show_payment(self):
        self.enter_payment.delete(0, tk.END)

        # set answer to interest payments
        self.mtg_calc.compute_monthly_payment()
        self.answer = round(self.mtg_calc.get_payment(), 2)
    ...
    def end_my_gui(self):
        ...
        if tkmb.askokcancel("End GUI", confirm_quit):
    ...
    def open_options_win(self):
        ...
        container = tk.Frame(self.options_win, bg=self.WRK_CLR, padx=5)
        title_frame = tk.Frame(container, bg=self.WRK_CLR)
        work_frame = tk.Frame(container, bg=self.OPT_CLR_RAD)
        button_frame = tk.Frame(container, bg=self.BTN_CLR)

        # -------------- message widget for title ---------------
        instructions = "Select an unknown value to be computed from 3 others"
        msg_head = tk.Message(title_frame, text=instructions)
        msg_head.config(font=("Helvetica Neue", 12),
                        bg=self.BTN_CLR, width=450)

        # -------------- radio btn widget for op ---------------
        op_buts = [tk.Radiobutton(work_frame, text=unk[k].name,
                                  variable=self.state_cur_unk_int,
                                  value=unk[k].value, bg=self.OPT_CLR_RAD, padx=5, pady=10
                                  ) for k in range(4)]

        # ----------------- some button widgets -------------------
        but_save = tk.Button(button_frame, text="Save",
                             command=options_save, bg="ghostwhite")
        but_cancel = tk.Button(button_frame, text="Cancel",
                               command=options_cancel, bg="ghostwhite")
        but_about = tk.Button(button_frame, text="About",
                              command=about_info, bg="ghostwhite")
        ...
        for k in range(len(self.MtgVars)):
            op_buts[k].pack(side=tk.LEFT, padx=20)

        but_save.pack(side=tk.LEFT, padx=10, pady=10)
